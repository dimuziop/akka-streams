...
class TestingStreamsSpec extends TestKit(ActorSystem("TestingAkkaStreams"))
  ...
  "A simple stream" should {
    "satisfy basic assertions" in {
      val aSimpleSource = Source(1 to 10)
      val simpleSink = Sink.fold(0)((a: Int, b: Int) => a + b)

      val sumFuture = aSimpleSource.toMat(simpleSink)(Keep.right).run()
    ...
    "integrate with test actors via materialized values" in {
      ...
      aSimpleSource.toMat(simpleSink)(Keep.right).run().pipeTo(probe.ref)
    ...
    "integrate with a test-actor-based sink" in {
      ...
      val aSimpleSource = Source(1 to 5)
      val flow = Flow[Int].scan(0)(_ + _) // 0, 1 , 3, 6 , 10 ,15

      val streamUnderTest = aSimpleSource.via(flow)

      val probe = TestProbe()
      val probeSink = Sink.actorRef(probe.ref, "completed", err => s"Failed due to $err")
